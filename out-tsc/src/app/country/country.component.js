...
var CountryComponent = /** @class */ (function () {
    function CountryComponent(_mySqlService, _http) {
    ...
    CountryComponent.prototype.onDeleteConfirm = function (event) {
        ...
        if (window.confirm('Are you sure you want to delete?')) {
            this._http.post('http://localhost:6600/deleteCountry', event.data).subscribe(function (res) {
                _this.deleteMsg = true;
                _this.deletedMsg = "Country details of  ID " + " ' " + event.data.id + " ' " + " deleted successfully";
                setTimeout(function () {
    ...
    CountryComponent.prototype.onCreateConfirm = function (event) {
        var _this = this;
        this._http.post('http://localhost:6600/saveCountry', event.newData).subscribe(function (res) {
            _this.displayMsg = true;
            _this.successMsg = "Country details of ID" + " ' " + event.newData.id + " ' " + " created successfully ";
            setTimeout(function () {
    ...
    CountryComponent.prototype.onSaveConfirm = function (event) {
        var _this = this;
        this._http.post('http://localhost:6600/updateCountry', event.newData).subscribe(function (res) {
            _this.updateMsg = true;
            _this.updatedMsg = "Country details of  ID" + " ' " + event.newData.id + " ' " + " updated successfully ";
            setTimeout(function () {
